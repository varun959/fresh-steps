...
function TypeBadge({ type }: { type: 'loop' | 'one-way' | 'out-and-back' }) {
  ...
  return (
    <span className="relative inline-flex items-center gap-1 group">
      ...
      <span className="pointer-events-none absolute bottom-full left-0 mb-1.5 w-48 rounded-lg bg-gray-800 text-white text-xs px-2.5 py-1.5 z-50 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
        {tooltip}
      </span>
    </span>
  )
